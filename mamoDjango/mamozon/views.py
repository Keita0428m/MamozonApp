...
class SignUp(generic.CreateView):
    """ユーザー仮登録"""
    template_name = 'mamozon/sign_up.html'
    ...
    def form_valid(self, form):
        ...
        subject_template = get_template('mamozon/mail_template/sign_up/subject.txt')
        subject = subject_template.render(context).strip()

        message_template = get_template('mamozon/mail_template/sign_up/message.txt')
        message = message_template.render(context).strip()

        user.email_user(subject, message)
        messages.success(self.request, '本登録用リンクを送付しました')
        return  HttpResponseRedirect(reverse('mamozon:sign_up'))

class SignUpDone(generic.TemplateView):
    """メール内URLアクセス後のユーザー本登録"""
    template_name = 'mamozon/sign_up_done.html'
...
class ShoppingCartDetail(LoginRequiredMixin, generic.DetailView):
    model = ShoppingCart
    template_name = "mamozon/shopping_cart.html"
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(exist) > 0:
            ...
            new_cart_item.save()
        return HttpResponseRedirect(reverse('mamozon:cart',  kwargs={'pk': self.get_object().pk}))
